...
namespace eAgenda.Controladores.CompromissoModule
{
    public class ControladorCompromisso : Controlador<Compromisso>
    {
        ...
        public override string Editar(int id, Compromisso registro)
        {
            ...
            if (!ValidarConflitos(registro.Data, registro.HoraInicio, registro.HoraTermino))
                resultadoValidacao = "Hora inválida! tente novamente...";
        ...
        public bool ValidarConflitos(DateTime data, TimeSpan horaInicio, TimeSpan horaTermino)
        {
            foreach (Compromisso c in SelecionarTodos())
            {
                if (c.Data.CompareTo(data) == 0)
                {
                    bool horaInicioInvalida = c.HoraInicio.CompareTo(horaInicio) < 0 && c.HoraTermino.CompareTo(horaInicio) > 0;
                    bool horaTerminoInvalida = c.HoraInicio.CompareTo(horaTermino) < 0 && c.HoraTermino.CompareTo(horaTermino) > 0;
                    bool horaInicioTerminoInvalida = c.HoraInicio.CompareTo(horaInicio) > 0 && c.HoraTermino.CompareTo(horaTermino) < 0;
