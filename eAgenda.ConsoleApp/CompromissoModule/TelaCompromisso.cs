    ...
    public class TelaCompromisso : TelaCadastroBasico<Compromisso>, ICadastravel
    {
        ...
        public override Compromisso ObterRegistro(TipoAcao tipoAcao)
        {
            ...
            while (true)
            {
                ...
                horaInicio = new TimeSpan(int.Parse(strHoraInicio[0]), int.Parse(strHoraInicio[1]), 0);

                Console.Write("Digite a hora de inicio do compromisso [12:00]: ");
                string[] strHoraFim = Console.ReadLine().Split(':');

                horaFim = new TimeSpan(int.Parse(strHoraFim[0]), int.Parse(strHoraFim[1]), 0);

                if (controladorCompromisso.ValidarConflitos(data, horaInicio, horaFim))
                    break;
                else
                    ApresentarMensagem("Hora inválida! tente novamente...", TipoMensagem.Atencao);
            }
